...
# data_raster <- rasterFromXYZ(df_sst, crs = "EPSG:4326") # vers un RasterLayer

df_sst <- df_sst |> mutate(lon = if_else(lon > 180, lon - 360, lon))
data_raster <- terra::rast(df_sst, crs = "EPSG:4326")
cl <- as.contour(data_raster) |> st_as_sf()
plot(data_raster, axes = TRUE)
plot(world, add = TRUE)
plot(cl, add = TRUE)

raster_wintri <- project(data_raster, "+proj=wintri", method = "near")
world_wintri = st_transform(world, crs = "+proj=wintri")
plot(st_geometry(world_wintri))
plot(raster_wintri, add = TRUE)

rasterVis::contourplot(data_raster, contour = TRUE, labels = FALSE)
```
