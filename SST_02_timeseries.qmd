...
# Il est nécessaire de donner des limites géographiques 
# sous forme de coordonnées d'un polygone différent d'un carré !!!!

# geo_mediterranean_sea <- list(
#   RDS_name = "mediterranean_sea",
#   box = c(lon_min = 280, lon_max = 360, lat_min = 0, lat_max = 60)
# )
# 
# df_wm_sst_mediterranean_sea <- get_var_all_weighted_mean(
#   "sst", geo_mediterranean_sea)
# df_wm_anom_mediterranean_sea <- get_var_all_weighted_mean(
#   "anom", geo_mediterranean_sea)
# df_wm_err_mediterranean_sea <- get_var_all_weighted_mean(
#   "err", geo_mediterranean_sea)
# df_wm_ice_mediterranean_sea <- get_var_all_weighted_mean(
#   "ice", geo_mediterranean_sea)
...
df_wm_sst_world <- readRDS('wm_sst_world.RDS') |> 
  mutate(
    year = year(date),
    yday = yday(date),
    month = month(date)
...
ggplot(data = df_wm_sst_world,
       aes(x = yday, y = wm_sst, group = year)) +
  geom_line(aes(color = factor(year))) +
  scale_color_grey(start = 0.8, end = 0.2) +
  geom_line(
    data = df_wm_sst_world |> filter(year == 2023),
    aes(x = yday, y = wm_sst),
    color = "orange",
    linewidth = 1) +
  geom_line(
    data = df_wm_sst_world |> filter(year == 2024),
    aes(x = yday, y = wm_sst, group = year),
    color = "black", 
    linewidth = 1) +
  labs(
